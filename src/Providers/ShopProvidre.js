...
const ShopProvider = ({children}) => {
    ...
    const [increase, setincrease] = useState({ amuont :'' , state : false,count:0})
    const [ decrease, setdecrease] = useState({ amuont :'' , state : false,count:0})
    ...
    const API_Shop= {
        ...
        DeleteItemCart   : async (params,onSuccess) => await API.post('user/delete-prod-cart', params).then(res => {Alert.alert(res.data.message) }),
        ConfirmOrder : async (params,onSuccess) => await API.post('user/create-order', params).then(res => {setConfirmOrderData(res.data) ,console.log(res.data , "setConfirm ++++++++======>")}),
